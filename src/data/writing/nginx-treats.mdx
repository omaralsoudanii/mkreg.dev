...
> If you are curios about the layers and their numbering, it's based on the OSI model. You can read about it in details at [OSI model - Wikipedia](https://en.wikipedia.org/wiki/OSI_model)
...
- #### Proxy buffering
  ...
  - After reaching point #3 if the response doesn’t fit in the buffer size I allocated, NGINX would have to save part of it on disk temporary, send the rest then read the temp file and send the remaining. This can be tuned by allowing NGINX to allocate everything in the memory instead for this particular case, this is up to you on how you control it but I prefer keeping the memory stable and let NGINX write temp files for those edge cases.
     ...
     [Proxy buffering Documentation](https://nginx.org/en/docs/http/ngx_http_proxy_module.html#proxy_buffering)

	

The following will is not covered here, maybe I'll write another article dedicated for them
...
Actually, you know what? you could also start from the [configuration root folder](https://github.com/omaralsoudanii/mkreg.dev/tree/with-nginx/docker/nginx) and read the rest. Some weird things used as a workaround for handling NextJs bad headers, also this instance is backed by HAProxy, so there this is some proxy protocol stuff.
...
- #### The host IP address
  ...
  I'm definitely not a security expert, but I don't want the server IP address that NGINX run to become accessible whether it's from browser or something like Curl. For NGINX after some research solutions I found basically covers the non-HTTP part, in another words if you type the IP address in your browser instead of responding with the application, NGINX will return whatever error or page you decide.

  However for HTTPS I found nothing, if you type the IP address with the addition of HTTPS or port 443 it doesn't work.

  > An example (with dummy IP) would be something like this:
  >
  > 78.555.666.33
  >
  > http://78.555.666.33
  ...
  > https://78.555.666.33
   ...
   The response would be returned from the browser with the known **Invalid certificate error page**
...
- Dealing SSL certificate, from the issuing the cert to creating a configuration optimized for performance while maintaining security, and a bonus auto renewal functionality.
- Compression algorithms, Brotli and Gzip
- Caching static and dynamic responses via different NGINX modules
- Docker setup
