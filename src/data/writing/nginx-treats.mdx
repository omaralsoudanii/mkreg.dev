...
Although it’s NGINX specific — web servers tend to share some common behavior and patterns. Reading this will provide you with a different ideas when building and tuning your server whether you are using NGINX or something different. 
...
After we provide the empty value variable for `ssl_certificate` and `ssl_certificate_key` with the addition of `ssl_ciphers aNULL`.
...
`value1` and `value2` will not be sent because the location block `add_header` directives overrides the previous configuration level which is `server`.
...
Separate configuration files so they can be reusable — a good practice is to create two files, one for the proxy headers and another for the client headers. Then simply include the file in the level you would want to apply the headers for. 
