...
Well it's either NGINX or HAProxy or both. I keep experimenting so this list is bound to change from time to time. If most of the site is served by Cloudflare and the revalidation is handled by a proxy server – then in this case I'm only concerned about [L4](https://en.wikipedia.org/wiki/OSI_model). Because the whole thing basically dependent on latency and response time – and ofcourse HAProxy is miles ahead in this area compared to NGINX.
